...
// GetSocialLinksBySourceDomain получает ссылки найденные на конкретном домене
func (s *SocialLinkService) GetSocialLinksBySourceDomain(ctx context.Context, sourceDomain string) ([]*ent.SocialLink, error) {
    return s.client.SocialLink.Query().
        Where(sociallink.SourceDomain(sourceDomain)).
        All(ctx)
}
